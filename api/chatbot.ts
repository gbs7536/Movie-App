...
const openai = new OpenAI({
  ...
  baseURL: 'https://api.groq.com/openai/v1' // 꼭 설정해야 Groq 사용 가능
})
...
async function sendMessages(messages: OpenAI.ChatCompletionMessageParam[]) {
  const fineTunedMessages: OpenAI.ChatCompletionMessageParam[] = [
    ...
    { role: 'assistant', content: '가벼운 분위기의 영화를 추천해드릴게요. {{비커밍 제인//becoming jane}}(2007)는 제인 오스틴의 청춘 시절을 다룬 멜로 영화로, 사랑과 운명을 그려낸 로맨틱한 작품이에요.' },
    ...
    { role: 'assistant', content: '크리스토퍼 놀란 감독이 연출한 {{인터스텔라//interstellar}}(2014)는 과학과 인간 정서를 균형 있게 결합한 공상 과학 장르의 대표작입니다.' },
